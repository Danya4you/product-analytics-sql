...
def chart_channels(rows) -> str:
    ...
    for i, r in enumerate(rows):
        ...
        s.append(f'<text class="value-label" x="{L + max(v * scale, 3) + 12:.1f}" '
                 f'y="{y + row_h / 2 + 5:.0f}">{mark} {v:.2f}</text>'.replace(".", ",", 1))
    s.append("</svg>")
    return "\n".join(s)
...
def build(psql: str) -> str:
    ...
    figures = [
        ...
        figure(
            ...
            + "LTV здесь модельная (постоянный отток, маржа 80 %) и завышена; "
            "выводы безопаснее делать по нижней оценке из "
            '<a href="../docs/metrics.md">словаря метрик</a>.',
